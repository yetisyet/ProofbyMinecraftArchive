...
class FOLe:
    ...
    def _AddBrackets(expr, left, right) -> str:
        ...
        if (left > 0 and right <= len(expr) and
            expr[left] == '(' and expr[right-1] == ')'):
            return expr
    ...
    def OrderOper(expr: str) -> str:
        ...
        #     left, right = FOLe._GetLocal(expr, i, True)
        ...
        for i in (idx):
            left, right = FOLe._GetLocal(expr, i, False)
            expr = FOLe._AddBrackets(expr, left, right)
